...
def generate_coarse_report(history_prompt, token_samples=3):
    ...
    if not isinstance(semantic_history, np.ndarray) and not isinstance(
        ...
        report["valid"] = False
        report["messages"].append(
            "should be a 1d numpy array but shape is {semantic_history.shape}."
        )
...
def generate_fine_report(history_prompt, token_samples=3):
    ...
    if not isinstance(fine_history, np.ndarray):
        report["valid"] = False
        report["messages"].append(
            "fine_prompt should be a numpy array but it's a {type(fine_history)}."
        )

    elif len(fine_history.shape) != 2:
        report["valid"] = False
        report["messages"].append(
            "fine_prompt should be a 2-dimensional numpy array but shape is {fine_history.shape}."
        )
...
def startup_status_report(quick=True, gpu_no_details=False):
    ...
    try:
        ...
        status += f"\nError getting FFmpeg directories. {str(e)}\n"
        status += """
        Try running:
        ffdl install -U --add-path 

        You can also install ffmpeg.exe in regular windows, and make sure the path is in your PATH environment variable.
        Basically you want to be able to type 'ffmpeg -version' in a command prompt and have it work. And it should be version 6.0
        """
