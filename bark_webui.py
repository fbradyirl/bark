...
def clone_voice_gradio(
    ...
    semantic_step_interval,
    dest_filename,
    create_samples_for_clones,
    even_more_clones,
):
    clone_dir = clone_voice(
        ...
        input_audio_filename_secondary,
        dest_filename,
        semantic_step_interval=semantic_step_interval,
...
def generate_audio_long_gradio(
    ...
    if npz_dropdown != "" and npz_dropdown is not None:
        if len(npz_dropdown.strip()) > 6:
            kwargs["history_prompt"] = npz_dropdown

    if bark_infinity_voices != "" and bark_infinity_voices is not None:
        if len(bark_infinity_voices.strip()) > 6:
            kwargs["history_prompt"] = bark_infinity_voices

    if generated_voices != "" and generated_voices is not None:
        if len(generated_voices.strip()) > 6:
            kwargs["history_prompt"] = generated_voices

    if cloned_voices != "" and cloned_voices is not None:
        if len(cloned_voices.strip()) > 6:
            kwargs["history_prompt"] = cloned_voices
...
sys.stdout = io.TextIOWrapper(
    sys.stdout.buffer, encoding="utf-8", errors="replace", newline="", line_buffering=True
)
sys.stderr = io.TextIOWrapper(
    sys.stderr.buffer, encoding="utf-8", errors="replace", newline="", line_buffering=True
)
...
model_options = [
    ...
    (
        "force_reload",
        {
            "value": False,
            "type": bool,
            "help": "Force the models to be loaded again. You have to do this if you already preloaded.",
        },
    ),
]
...
with gr.Blocks(theme=default_theme, css=bark_console_style) as demo:
            ...
            with gr.Tab("Text Prompts"):
                with gr.Row(elem_id=f"text_row"):
                    input = gr.TextArea(
                        ...
                        info="The main text goes here. It can be as long as you want. You will see how the text will be split into smaller chunks on the right.",
                    )

                with gr.Row(elem_id=f"styles_row"):
                    with gr.Column(variant="panel", scale=0.5):
                        prompt_styles_dropdown = gr.Dropdown(
                            label=f"Text Snippets",
                            info=f"Add your own! {user_style_csv}",
                            elem_id=f"styles",
                        ...
                        prompt_style_apply = ToolButton(
                            value=apply_style_symbol, elem_id=f"style_apply"
                        )
                        # save_style = ToolButton(value=save_style_symbol, elem_id=f"style_create")
                    with gr.Column(variant="panel", scale=0.5):
                        prompt_transformations_dropdown = gr.Dropdown(
                            label=f"Text Transformations",
                            info=f"Add your own! {user_transformation_csv}",
                            elem_id=f"transformations",
                            choices=[k for k, v in prompt_transformations.transformations.items()],
                            value=[],
                            multiselect=True,
                        )
                        # create_refresh_button(prompt_styles_dropdown, prompt_styles.reload, lambda: {"choices": [k for k, v in prompt_styles.styles.items()]}, f"refresh_styles")
                        prompt_transformations_apply = ToolButton(
                            value=text_transformation_symbol, elem_id=f"transformation_apply"
                        )
                        # save_style = ToolButton(value=save_style_symbol, elem_id=f"style_create")
            ...
            with gr.Tab("Audio/Semantic Prompts"):
                with gr.Row(elem_id=f"text_row"):
                    with gr.Column(variant="panel", scale=1.0):
                        gr.Markdown(
                            "Experimental, try using an audio clip as as the prompt instead of text. It's the text so you still pick a different speaker files to use. Audio less than 14 seconds."
                        )
                        ...
                        gr.Markdown(
                            "Try using a speaker .npz as the prompt, not the speaker. (So you can still pick a different speaker.)"
                        )
                        bark_speaker_as_the_prompt = gr.File(
                            label="Pick a Specific NPZ From Filesystem", file_types=["npz"]
                        )
        ...
        with gr.Row():
            with gr.Column(variant="panel", scale=0.75):
                ...
                with gr.Tab("🌱🎙️ Create New Speaker"):
                    ...
                    m(
                        "If you want a random speaker make sure to clear out all the dropdown menus. UI is work in progress."
                    )
                with gr.Tab("🧑‍🎤 Build In Speakers"):
                    npz_dropdown = create_npz_dropdown(
                        "bark/assets/prompts/",
                        label="Speaker",
                        info="These are speakers provided by Suno-ai, in many languages. The v2 ones are good for a basic clear voice.",
                    )
                    gr.Markdown("🌌🧙 Bark Infinity Speaker")
                    gr.Markdown(
                        """Speakers I accidentally left in in when I first made this. They will get better <a href='https://github.com/JonathanFly/bark/discussions/47'>contribute</a> to contribute."""
                    )
                    bark_infinity_voices = create_npz_dropdown(
                        "bark_infinity/assets/prompts/", label="Speaker"
                    )
                with gr.Tab("👩‍🎤🎙️ Your Custom Speaker"):
                    gr.Markdown("""Pick Specific speaker NPZ From Filesystem""")

                    specific_npz_file = gr.File(
                        label="Pick Specific NPZ From Filesystem", file_types=["npz"]
                    )
                    ...
                    generated_voices = create_npz_dropdown(
                        "custom_speakers/",
                        label="Speaker",
                        info="custom_speakers/ doesn't refresh yet but the Pick Specific speaker NPZ filepicker works.",
                    )

                    cloned_voices = create_npz_dropdown(
                        "cloned_voices/",
                        label="Cloned Voices",
                        info="cloned_voices/ doesn't refresh yet but the Pick Specific speaker NPZ filepicker works.",
                    )

                with gr.Tab("👩‍🎤🎙️ Folder Processing"):
                    gr.Markdown(
                        """Generation a sample for every npz in the directory. If you're testing clones, check the Save Every NPZ checkbox as well."""
                    )
            ...
            with gr.Column(variant="panel", scale=0.25):
                ...
                with gr.Tab("Simple"):
                    ...
                    split_character_goal_length = gr.Slider(
                        label="Try for this many characters in each", value=165, maximum=500, step=1
                    )
                    split_character_max_length = gr.Slider(
                        label="But never go higher than this many", value=205, maximum=500, step=1
                    )

                with gr.Tab("Fancy"):
                    prompt_text_prefix = gr.Textbox(
                        label="Put this text in front of every prompt, after splitting.", value=""
                    )
                    ...
                    process_text_by_each = gr.Dropdown(
                        ["word", "line", "sentence", "char", "string", "random", "regex"],
                    ...
                    split_type_string = gr.Textbox(
                        label="(Optional String for string or regex.)", value=""
                    )

                text_splits_only = gr.Checkbox(
                    label="🗺️✂️ No audio, just show me text splits.", value=False
                )
            ...
            with gr.Column(variant="panel", scale=0.25):
                ...
                with gr.Tab("Simple"):
                    ...
                    stable_mode_interval = gr.Dropdown(
                        ["Continuous", "Stable", "Stable-2", "Stable-3", "Stable-4", "Stable-5"],
                ...
                with gr.Tab("Fancy"):
                    semantic_min_eos_p = gr.Slider(
                        label="How likely the clip will end.",
                        minimum=0.0,
                        maximum=1.0,
                        value=0.2,
                        interactive=True,
                        info="Try 0.10 or 0.05 if you're getting extra words.",
                    )
                    ...
                    history_prompt_semantic_weight = gr.Slider(
                        label="History Prompt Semantic Weight",
                        minimum=0.0,
                        maximum=2.0,
                        value=1.0,
                        interactive=True,
                    )
                    history_prompt_coarse_weight = gr.Slider(
                        label="History Prompt Coarse Weight",
                        minimum=0.0,
                        maximum=2.0,
                        value=1.0,
                        interactive=True,
                    )
                    history_prompt_fine_weight = gr.Slider(
                        label="History Prompt Fine Weight",
                        minimum=0.0,
                        maximum=2.0,
                        value=1.0,
                        interactive=True,
                    ...
                    prev_semantic_weight = gr.Slider(
                        label="Prev Semantic Weight",
                        minimum=0.0,
                        maximum=2.0,
                        value=1.0,
                        interactive=True,
                    )
                    prev_coarse_weight = gr.Slider(
                        label="Prev Coarse Weight",
                        minimum=0.0,
                        maximum=2.0,
                        value=1.0,
                        interactive=True,
                    )
                    prev_fine_weight = gr.Slider(
                        label="Prev Fine Weight",
                        minimum=0.0,
                        maximum=2.0,
                        value=1.0,
                        interactive=True,
                ...
                with gr.Tab("Don't Connect Them"):
                    m(
                        "### Split the text, but treat each segment like it's own prompt. Use a random speaker for each segment."
                    )
                    m("Good for discovering speakers.")
                    seperate_prompts = gr.Checkbox(
                        label="Seperate Prompts", value=False, interactive=True, visible=True
                    )

                    m(
                        "Same, but keep the history one time so you get a better idea of what the speaker will actually do."
                    )
                    seperate_prompts_flipper = gr.Checkbox(
                        label="Seperate Prompts, but do one generation",
                        value=False,
                        interactive=True,
                        visible=True,
                    )
            ...
            with gr.Column(variant="panel", scale=0.25):
                ...
                with gr.Tab("Simple"):
                    ...
                    confused_travolta_mode = gr.Checkbox(
                        label="🕺🕺 Confused Travolta Mode",
                        info="Always generate maximum length. Great for history prompts of music and sounds.",
                        value=False,
                    )
                with gr.Tab("Fancy"):
                    seed = gr.Number(
                        label="Random SEED: 0 for no seed. Set -1 to undo.",
                        info="(Bark runs a lot slower when using a seed.)",
                        value=0,
                    )
                    m(
                        """## These options should in theory have a decent impact
                    ...
                    semantic_top_k = gr.Slider(
                        label="semantic_top_k", value=0, minimum=0, maximum=300, step=1
                    )
                    semantic_top_p = gr.Slider(
                        label="semantic_top_p", value=0, minimum=0.0, maximum=1.0
                    )
                    coarse_top_k = gr.Slider(
                        label="coarse_top_k", value=0, minimum=0, maximum=300, step=1
                    )
                    coarse_top_p = gr.Slider(
                        label="coarse_top_p", value=0, minimum=0.0, maximum=1.0
                    )
                    m(
                        """## Anecdotally:
            ...
            with gr.Column(variant="panel", scale=0.25):
                ...
                with gr.Tab("Simple"):
                    ...
                    output_iterations = gr.Slider(
                        label="Repeat This Many Times", step=1, value=1, minimum=1, maximum=1000
                    )
                with gr.Tab("Fancy"):
    ...
    with gr.Tab("Model Options, Configs, Setup"):
        with gr.Row():
            gr.Markdown(
                "Preloading is a little bit faster if you have enough GPU memory, but the difference is actutally pretty small. You can still use the larger models just fine without preloading them, they get swapped out to CPU by default in this version."
            )
        with gr.Row():
            with gr.Column(scale=0.25, variant="panel"):
            ...
            with gr.Column(scale=0.75, variant="panel"):
                gr.Markdown("## System Wide Settings")
                gr.Markdown("If you have 10GB of VRAM")
                m(
                    "Then click 'Set these parameters' with OFFLOAD_CPU unchecked. If you already preloaded models, preload again with force_reload=True."
                )
                ...
                env_config_group = gr.CheckboxGroup(
                    ...
                    label="System Wide Config Settings",
                    type="value",
                    interactive=True,
                    visible=True,
                )
                ...
                env_input_list = (
                    [env_config_group] + [loglevel, save_log_lines_number] + model_checkboxes
                )
    ...
    with gr.Tab("# HuBERT Voice Clone 👨‍🔬🧬🔁👯‍♂️🌌 via https://github.com/gitmylo"):
        ...
        with gr.Row():
            with gr.Column(scale=0.5, variant="panel"):
                ...
                input_audio_filename = gr.Audio(
                    label="Upload Your Audio File, Most Common Audio Forms",
                    info="",
                    source="upload",
                    type="filepath",
                )
                ...
                gr.Markdown(
                    """### Use audio as long or short audio you like, but for now stick to a few minutes at most, for memory reasons. It's typically better if your audio has a natural pause at the end, but not absolutely necessary."""
                )

                gr.Markdown(
                    """### Note that this version if upload tons of audio you are probably not giving Bark more data in the sense you think. This will fill your hard drives with different voice clones."""
                )

                gr.Markdown(
                    """The longer audio is not being used to train a model or really be referenced referenced as a whole. If you upload a minute minutes of audio, you will get a speaker created every few seconds in that audio. Effectively this is what you would have gotten if had cut up that 30 seconds into tiny clips and uploaded them one by one, to make a clone of each. (It is actually a bit better though - it uses overlapping clips intsead of splitting."""
                )
                ...
                gr.Markdown(
                    """A natural pause at the end of a short clip can improve the cloning output.
                ...
                gr.Markdown(
                    """Don't get too excited listening to the audio labelled 'raw' in the clone directory. That's just copy of the raw audio you uploaded, useful to knowing what's in each segment. You won't get such a close voice using the files. """
                )
            ...
            with gr.Column(scale=0.50, variant="panel"):
                ...
                semantic_step_interval = gr.Slider(
                    label="Space between audio clones segments in the files",
                    info="If you've only got a short sample or you feel like you just just barely missing a good voice, you can try lower values. On the default each speak already overlaps a lot. For very long clips, very high numbers will just take a few samples.",
                    step=1,
                    value=164,
                    maximum=10000,
                    minimum=32,
                ...
                create_samples_for_clones = gr.Checkbox(
                    label="Create an audio sample for each created clone at the end, using the Main Text Prompt",
                    value=False,
                )
                gr.Markdown(
                    """Make sure you put text in the main text prompt. Take time to get text that is has the style and rhythm the voice you want to tclnoe, it will save after each sample, they often work well as clones."""
                )

                initialname = "NewClonedVoiceName"
                output_voice = gr.Textbox(
                    label="Name Your Cloned Voice",
                    lines=1,
                    placeholder=initialname,
                    value=initialname,
                    info="The output file will be stored in ./cloned_voices/, along with a copy of the original audio. Retaining the default name won't overwrite existing files but will add a numerical suffix.",
                )

                even_more_clones = gr.Checkbox(
                    label="Just give me more clones. 😱💡➡️🧪🧬👯‍♂️👯‍♀️ This is not a great progress but it does get lucky once in awhile.",
                    value=False,
                )

                even_more_clones_power_level = gr.Slider(
                    label="If you check 'more clones' this will repeat the generations a few more times. Very slow, each one takes a long as a clip to renders.",
                    step=1,
                    value=1,
                    maximum=100,
                    minimum=1,
        ...
        with gr.Row():
            clone_voice_button = gr.Button(
                "Begin Generating Voice Clones", variant="primary", elem_id="cloning"
            )
        ...
        with gr.Row():
            with gr.Column(scale=0.25):
                ...
                sample_gen_path = gr.Textbox(
                    label="Sample Directory", value="bark/assets/prompts/v2"
                )
                ...
                gr.Markdown(
                    "Generate Slight Variations. These will sound almost but not quite the same as original. Not particularly useful honeslty."
                )
                sample_gen_button_2 = gr.Button(
                    "Generate Slight Variations.", info="", variant="primary"
                )
                sample_gen_button_2.click(generate_sample_audio_coarse, inputs=sample_gen_path)

                gr.Markdown(
                    "Generate Wild Variations. These are wildly different from the original. They may not be the same gender. This is a decent way to find differnet but somewhat similar voices, but it's not the that useful either."
                )
                sample_gen_button_3 = gr.Button(
                    "Wildly Different Samples", info="Wildly Different samples", variant="primary"
                )
            ...
            with gr.Column(scale=0.50):
                ...
                with gr.Tab("### Doctor RNG 👩🏻‍⚕️🎲"):
                    ...
                    variation_path = gr.Textbox(
                        label="Speaker NPZ Path", value="bark_samples/myspeakerfile.npz"
                    )
                    ...
                    generate_speaker_variations_button.click(
                        generate_speaker_variations, inputs=[variation_path, variation_count]
                    )
    ...
    with gr.Tab("More Options"):
        with gr.Row():
            with gr.Column(scale=0.33, variant="panel"):
                m("# 🐍🐍 Advanced Options")
            ...
            with gr.Column(scale=0.33, variant="panel"):
                ...
                gr.HTML(
                    ...
                    info=". I cut a lot of these out becaus they were buggy or took too long to try and merge with regular Bark because I don't really understand the stuff I poke at very well.",
                )
            with gr.Column(scale=0.33, variant="panel"):
                extra_args_input = gr.TextArea(
                    lines=15, label="Extra Arguments", elem_classes="bark_console"
                )

    with gr.Row():
        with gr.Column(scale=1):
            generate_button = gr.Button(
                "Generate Audio! (Not a Clone, that's on a different tab.)", variant="primary"
            )

        with gr.Column(scale=1):
            cancel_button = gr.Button(
                "Cancel. It works, just hit it once and give it few seconds.",
                label="",
                variant="stop",
            )
    ...
    cancel_button.click(
        fn=try_to_cancel, inputs=model_checkboxes, outputs=None, cancels=[generate_event]
    )
